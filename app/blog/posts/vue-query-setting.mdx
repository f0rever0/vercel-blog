...
만약 아래 예시의 `useTodos()` 를 여러 컴포넌트에서 사용하고 있을때 useTodo 호출시 에러가 발생한다면 useTodos를 사용하는 횟수 만큼 에러가 발생할 것입니다.
...
useQuery의 onError 콜백은 모든 Observer에 대해 호출되므로, 애플리케이션에서 useTodos를 두 번 호출하면 네트워크 요청이 한 번만 실패하더라도 두 번의 에러 토스트가 뜨게 됩니다.
